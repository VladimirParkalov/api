...
<?php if( isset($records) && is_object($records) && $records->count() > 0 ): ?>
  ...
  $noAttr = $record->level == 0 ?  $noRootAttr : $nodeNoAttr;
  ...
  <?php if($record->level > $prevLevel)  echo '<ul>'; elseif ($record->level < $prevLevel) echo str_repeat('</ul></li>', $prevLevel - $record->level); ?>
  ...
  <?php if ($record->level == 0) echo image_tag('/sfDoctrineTreePlugin/images/indicator.gif', array('style' => 'padding:0pt 5px;display:none;vertical-align:middle;', 'id' => 'doctrine_tree_indicator')) ?>
